...
class Main(tk.Frame):
	...
	def init_main(self):       ##############                D R A W I N G        S C R E E N
		...
		self.draw_toolbar()
										#      T A B L E
		self.tree = ttk.Treeview(self, columns=('ID', 'num', 'shipping', 'link', 'FIO', 'product', 'quantity', 'payment', 'net', 'order_date', 'shipping_date', 'shipping_way'), height=TABLE_HEIGHT, show='headings') # show=... - not to show 0 columns
		self.draw_scrollbar()
		self.draw_columns()
	...
	def draw_columns(self):
		...
		self.tree.column('quantity', width=QUANTITY_COLUMN, anchor=tk.CENTER)
...
# TODO                ##################   MANY PRODUCTS ??????????????
	...
	def records(self, shipping, link, FIO, product, payment, net, order_date, shipping_date, shipping_way):   # write new information
		self.db_people.c.execute('''SELECT id_client FROM people WHERE link = ? AND FIO = ?''', (link, FIO))  # try to find this client
		list_res = self.db_people.c.fetchall()
		if list_res:    # we already have this client
			new_id_client = list_res[0][0]
		else:
			self.db_people.insert_data(link, FIO)    # write new client
		...
		self.db_orders.insert_data(new_id_client, shipping, payment, net, order_date, shipping_date, shipping_way)
		self.db_orders.c.execute('''SELECT last_insert_rowid()''')
		order_id = self.db_orders.c.fetchall()[0][0]

		self.db_products.c.execute('''SELECT id_product FROM products WHERE name=?''', (product,))
		id_product = self.db_products.c.fetchall()[0][0]     # TODO ADD ERROR - product not found
		self.db_cart.insert_data(order_id, id_product, 1)   # TODO CHANGE HERE FOR MANY PRODUCTS
		self.view_records()
	...
	def view_records(self):
		...
		num_record = 0
		self.db_orders.c.execute('''SELECT * FROM orders''')
		for row in self.db_orders.c.fetchall():
			...
			self.db_cart.c.execute('''SELECT id_product, quantity FROM cart WHERE id_order=?''', (row[0],))
			id_product, quantity = self.db_cart.c.fetchall()[0];

			self.db_products.c.execute('''SELECT name FROM products WHERE id_product=?''', (id_product,))   # MANY PRODUCTS
			prod_list = self.db_products.c.fetchall()                     # TODO ADD ERROR
			prod_name=" - "
			if prod_list:
				prod_name = prod_list[0][0]
			res_row.append(prod_name)
			res_row.append(str(quantity))

			col = -1
			for t in row:    # other columns (date etc)
				col += 1
				if col in range(3): # to start from payment that is the 3rd column
					continue
	...
	def update_record(self, shipping, link, FIO, product, payment, net, order_date, shipping_date, shipping_way):   # to change values: UPDATE changes them. Then description...=? - what to change, WHERE ID - which line
			# line was changed => client could be changed => if this client had only one order (this one) we should delete him from db_people
			# first we will extract this client (by his id_client) and then we will count his orders
		self.db_orders.c.execute('''SELECT id_client FROM orders WHERE id_order=?''', (self.tree.set(self.tree.selection()[0], '#1')))  # #1 - column №1 - ID
		id_client = self.db_orders.c.fetchall()[0][0]

		self.db_orders.c.execute('''SELECT COUNT(*) FROM orders WHERE id_client=?''', (id_client,))
		orders = int(self.db_orders.c.fetchall()[0][0])
		...
		self.db_orders.c.execute('''UPDATE orders SET id_client=?, shipping=?, payment=?, net=?, order_date=?, shipping_date=?, shipping_way=? WHERE id_order=?''', (id_client, shipping, payment, net, order_date, shipping_date, shipping_way, self.tree.set(self.tree.selection()[0], '#1')))  # #1 - column №1 - ID
		self.db_orders.conn.commit()

		self.db_products.c.execute('''SELECT id_product FROM products WHERE name=?''', (product,))
		l = self.db_products.c.fetchall()
		if l:
			id_product = l[0][0]
		else:
			id_product = -1  # TODO ADD ERROR: there is no such product => DO SMTH
		self.db_cart.c.execute('''UPDATE cart SET id_product=?, quantity=1 WHERE id_order=?''', (id_product, self.tree.set(self.tree.selection()[0], '#1')))
		self.db_cart.conn.commit()
	...
	def delete_records(self):
		...
		if answ == True:
			...
			for selection_item in sel:   # selection returns numers of all chosen records
				self.db_orders.c.execute('''SELECT id_client FROM orders WHERE id_order=?''', (self.tree.set(selection_item, '#1')))
				id_to_delete = self.db_orders.c.fetchall()[0][0]
				self.db_orders.c.execute('''SELECT COUNT(*) FROM orders WHERE id_client=?''', (id_to_delete,))    # if this client had only one order => delete from people
				orders = self.db_orders.c.fetchall()[0][0]
				if orders == 1:   # delete from people
					self.db_people.c.execute('''DELETE FROM people WHERE id_client=?''', (id_to_delete,))
					self.db_people.conn.commit()
				self.db_orders.c.execute('''DELETE FROM orders WHERE id_order=?''', (self.tree.set(selection_item, '#1')))   # #1 - column from which we should take a number (column 1 because id is at this column)
				self.db_cart.c.execute('''DELETE FROM cart WHERE id_order=?''', (self.tree.set(selection_item, '#1')))
			self.db_orders.conn.commit()    # To save all results
			self.db_cart.conn.commit()
	...
	def to_excel(self):     # Выгрузить данные в excel
		...
		self.db_orders.c.execute('''SELECT * FROM orders''')
		table = self.db_orders.c.fetchall()

		num_record = 0
		main_list = [["ID", "Num", "Shipping", "Link", "Name", "Order", "Quantity", "Sum", "Net", "order_date", "shipping_date", "shipping_way"]]

		for row in table:
			num_record += 1
			cur_list = [row[0], num_record, row[2]]    # id_order, num, shipping (row[1] - id_client)
			...
			if prod_list:
				prod_name = prod_list[0][0]
			cur_list.append(prod_name)
			cur_list.append(quantity)
